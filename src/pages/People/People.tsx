...
const People = () => {
  ...
  useEffect(() => {
    const fetchUsersAndReferrals = async () => {
      try {
        const [topReferersResponse, topEarnersResponse] = await Promise.all([
          fetch('https://coinfarm.club/api/referral/top-referers'), // Запрос на топ-100 по рефералам
          fetch('https://coinfarm.club/api/user/top-earners') // Запрос на топ-100 по заработку
        ]);

        const topReferersData: User[] = await topReferersResponse.json();
        const topEarnersData: User[] = await topEarnersResponse.json();

        // Устанавливаем данные в состояние
        setUsersByReferrals(topReferersData);
        setUsersByEarnings(topEarnersData);
      } catch (error) {
        console.error('Failed to fetch users or referrals', error);
      }
    };
