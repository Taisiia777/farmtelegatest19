...
//   return <canvas ref={canvasRef} style={{ position: 'fixed', top: 0, left: 0, pointerEvents: 'none', zIndex: '13' }} />;
// };
  ...
  useEffect(() => {
    ...
    function confettiParticle(): Particle {
      const particle: Particle = {
        ...
        color:
          possibleColors[Math.floor(Math.random() * possibleColors.length)],
        tilt: Math.floor(Math.random() * 33) - 11,
        tiltAngleIncremental: Math.random() * 0.2 + 0.1, // Увеличение скорости наклона
        tiltAngle: 0,
        draw: function () {
          context.beginPath();
          context.lineWidth = this.r / 2;
    ...
    function Draw() {
      ...
      for (let i = 0; i < maxConfettis; i++) {
        ...
        particle.tiltAngle += particle.tiltAngleIncremental;
        particle.y += (Math.cos(particle.d) + 5 + particle.r / 2) / 1.5; // Увеличение скорости падения
        particle.tilt = Math.sin(particle.tiltAngle - i / 3) * 15;
