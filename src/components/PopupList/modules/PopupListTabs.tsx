         ...
         <ul className={cn("tabs__list")}>
            {labels.map((label) => (
               <li
                  ...
                  key={label}>
<span className="textMenu" style={{ height: labels.length === 3 ? '20%' : '73%', top: labels.length === 3 ? '40% !important' : '0', fontSize: labels.length === 3 ? '15px' : '20px' }}>{t(`${label.toLocaleLowerCase()}`)}</span>
