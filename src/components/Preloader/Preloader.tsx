...
const Preloader = () => {
  ...
  useEffect(() => {
    dispatch(loadingToggle(true)); // Изначально прелоудер включен
    calculateDaysLeft();
  }, [dispatch]);
  ...
  useEffect(() => {
    if (isQueryLoading) {
      dispatch(loadingToggle(true)); // Включаем прелоудер во время загрузки данных
    } else {
      dispatch(loadingToggle(false)); // Отключаем прелоудер после загрузки
    }
