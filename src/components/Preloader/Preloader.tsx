...
const fetchReferralsAndEarnings = async (userId: number) => {
   console.log(userId)
  const referralsResponse = await fetch(`https://coinfarm.club/api/user/3380/referrals`);
  if (!referralsResponse.ok) {
    throw new Error('Failed to fetch referrals');
  }
  const referralsData = await referralsResponse.json();

  const earningsResponse = await fetch(`https://coinfarm.club/api/user/3380/referrals/earnings`);
